...
export class SignInPage {
  ...
  public async logIn(userName: string, passwd: string): Promise<void> {
    await browser.wait(ExpectedConditions.presenceOf(this.userEle), 50000, 'It is not possible to access 1.');
    await this.userEle.sendKeys(userName);
    await browser.wait(ExpectedConditions.presenceOf(this.passwordEle), 50000, 'It is not possible to access 2.');
    await this.passwordEle.sendKeys(passwd);
    await browser.wait(ExpectedConditions.elementToBeClickable(this.loginBtnEle), 50000, 'It is not possible to access 3.');
